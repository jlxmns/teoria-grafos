...
public class Grafo {
    ...
    public int comprimentoCaminho(List<Vertice> verticesPercorridos) {
        if (!isPonderado) return verticesPercorridos.size() -1;

        int comprimento = 0;
        for (int i = 0; i < verticesPercorridos.size(); i++) {
            Vertice origem = verticesPercorridos.get(i);
            Vertice destino = verticesPercorridos.get(i + 1);
            Aresta aresta = encontraAresta(origem, destino);
    ...
    public int comprimentoCaminho(List<Aresta> arestasPercorridas) {
        if (!isPonderado) return arestasPercorridas.size();

        int comprimento = 0;
        for (Aresta aresta : arestasPercorridas) {
            comprimento += aresta.getPeso();
        }
    ...
    public List<Vertice> depthFirstSearch(Vertice vertice) {
        List<Vertice> visitados = null;
    ...
    private List<Vertice> search(List<Vertice> visitados, List<Vertice> caminho, Vertice vertice) {
        visitados.add(vertice);
        caminho.add(vertice);
    }
